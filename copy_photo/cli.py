...
from .utils import find_mount_point, verify_copy
from .file_operations import (
    find_photo_directories,
    get_earliest_photo_date,
    copy_with_progress,
    setup_destination
)
from .utils import verify_copy

def main():
    # Определяем путь к конфигу
    config_dir = os.path.expanduser("~/.config/copy-photo")
    ...
    try:
        ...
        print(f"Найдена флешки в: {mount_point}")

        # 2. Ищем директории с фотографиями
        photo_dirs = find_photo_directories(
            mount_point,
